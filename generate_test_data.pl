...
foreach my $wave_file(@wave_files)
{
	...
	while(my $row=<TRANSCRIPTION>)
	{
		...
		my $r=''; $r=~s/'//g; ## bluefish syntax fixer
		...
		foreach my $word(@words)
		{
			...
			$cmudict{"PHONEMES_$ph_range"}{$word}{'COUNT'}++; ## increment the occurrences number
...
for my $ph_range(reverse @phonemes_ranges)
{
	...
	while($cnt<$config{WORDS_NUMBER})
	{
		...
		foreach my $result(keys %{$cmudict{"PHONEMES_$ph_range"}})
		{
			...
			$most_frequent_count=$cmudict{"PHONEMES_$ph_range"}{$most_frequent_hash}{COUNT};
		}
		if($most_frequent_count ne 0) ## no more words for this phonemes count are present in the result
		{
